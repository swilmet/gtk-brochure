...
GTK+~3.22 --~released in September 2016~-- is and will be the latest GTK+ 3 version. GTK+~4 is in development, but a version of GTK+~4 with a stable API is planned only for 2018-2019. In the meantime, developers who want a stable API should use GTK+~3.22.

There will be snapshots of development of GTK+~4 every six months: 3.90, 3.92, 3.94, etc. Although it is only the beginning of GTK+~4 development, some highlights of what has been achieved so far:
\begin{itemize}
  \item Merge GSK, a scene graph API to take more advantage of the GPU for rendering the widgets, with OpenGL or Vulkan, and allowing 3D animations.
  \item Modernizing the GDK layer.
\end{itemize}
